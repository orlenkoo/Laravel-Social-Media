...
<p>A meeting you were invited to have been modified by {{ $activity_log->employee->given_name }} {{ $activity_log->employee->surname }}, Please check the details below and accept it.</p>
...
<p>
    <?php
    $url_accept = route('meeting_attendee.status.update', array('meeting_attendee_id' => $meeting_invitee->id, 'status' => 'Accept Edited', 'meeting_inviter_employee_id' => $activity_log->employee->id ));
    $url_reject = route('meeting_attendee.status.update', array('meeting_attendee_id' => $meeting_invitee->id, 'status' => 'Decline Edited', 'meeting_inviter_employee_id' => $activity_log->employee->id ));
